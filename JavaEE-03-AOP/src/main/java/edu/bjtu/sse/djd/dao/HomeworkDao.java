            ...
            try(PreparedStatement pstmt = connection.prepareStatement(sql)) {
                try (ResultSet resultSet = pstmt.executeQuery(sql)) {
                    ...
                    while (resultSet.next()) {
                        // 初始化一条结果
                        h = new Homework(resultSet.getLong("id"), resultSet.getString("title"),
                                resultSet.getString("content"), resultSet.getTimestamp("create_time"), resultSet.getTimestamp("update_time"));
                        resultList.add(h);
                    }
                }
            }
